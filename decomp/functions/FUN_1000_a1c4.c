/* 1000:a1c4 */

/* WARNING: Control flow encountered bad instruction data */

undefined4 FUN_1000_a1c4(void)

{
  byte *pbVar1;
  byte bVar2;
  undefined2 *puVar3;
  undefined2 *puVar4;
  byte bVar5;
  uint uVar6;
  byte bVar7;
  char extraout_AH;
  int iVar8;
  uint uVar9;
  uint uVar10;
  undefined2 in_CX;
  char cVar11;
  byte extraout_DL;
  int extraout_DX;
  undefined2 extraout_DX_00;
  int in_BX;
  byte *pbVar12;
  int unaff_BP;
  byte *unaff_SI;
  undefined2 *puVar13;
  int unaff_DI;
  undefined2 *puVar14;
  undefined2 unaff_SS;
  undefined2 unaff_DS;
  undefined1 uVar15;
  char cVar16;
  char cVar17;
  undefined1 *puVar18;
  int iVar19;
  
  puVar18 = &stack0x0002;
  func_0x0001eee3();
  bVar7 = *unaff_SI;
  *(uint *)(unaff_SI + in_BX + 0xe) =
       (*(int *)(unaff_SI + in_BX + 0xe) - extraout_DX) - (uint)CARRY1((byte)in_CX,*unaff_SI);
  pbVar12 = (byte *)CONCAT11(0xe8,(char)in_BX);
  pbVar1 = pbVar12 + unaff_DI + 1000;
  *(int *)pbVar1 = *(int *)pbVar1 + 1;
  if (CONCAT11((char)((uint)in_CX >> 8),(byte)in_CX + bVar7) != 1 && *(int *)pbVar1 != 0) {
                    /* WARNING: Bad instruction - Truncating control flow here */
    halt_baddata();
  }
  *(char *)0xb908 = *(char *)0xb908 + (char)in_BX;
  iVar19 = extraout_DX;
  func_0x000188bb();
  cVar17 = '\0';
  cVar16 = '\0';
  uVar15 = 1;
  cVar11 = extraout_AH;
  FUN_1000_59b2();
  func_0x00015bb8();
  bVar7 = FUN_1000_33bf();
  if ((bool)uVar15 || cVar17 != cVar16) {
    pbVar1 = pbVar12 + 0x3255;
    bVar2 = *pbVar1;
    bVar5 = *pbVar1 + bVar7 * -2;
    *pbVar1 = bVar5 - CARRY1(bVar7,bVar7);
    *(char *)(unaff_BP + 0x24ff) =
         *(char *)(unaff_BP + 0x24ff) + cVar11 +
         (bVar2 < (byte)(bVar7 * '\x02') || bVar5 < CARRY1(bVar7,bVar7));
    pbVar12[0x2055] = pbVar12[0x2055] ^ 0xb7;
    iVar8 = func_0x0001daad(puVar18);
    pbVar12[-0x4149] = pbVar12[-0x4149] | extraout_DL;
    iVar8 = iVar8 + 2;
    pbVar12[0x2055] = pbVar12[0x2055] + extraout_DL;
    (pbVar12 + -0x7318)[0] = 0x55;
    (pbVar12 + -0x7318)[1] = 0x20;
    *pbVar12 = *pbVar12 ^ (byte)pbVar12;
    iVar19 = *(int *)(pbVar12 + -0x3118);
    in(0xe8);
    FUN_1000_35c6();
    pbVar1 = (byte *)(in_BX + iVar19);
    bVar7 = *pbVar1;
    *pbVar1 = *pbVar1 + (byte)pbVar12;
    return CONCAT22(CONCAT11((char)((uint)extraout_DX_00 >> 8),
                             (char)extraout_DX_00 + pbVar12[iVar19 + 0x73e8] +
                             CARRY1(bVar7,(byte)pbVar12)),iVar8);
  }
  puVar13 = (undefined2 *)0x0;
  uVar6 = 0xa800;
  do {
    uVar9 = uVar6;
    puVar14 = (undefined2 *)0x0;
    for (iVar8 = 4000; iVar8 != 0; iVar8 = iVar8 + -1) {
      puVar4 = puVar14;
      puVar14 = puVar14 + 1;
      puVar3 = puVar13;
      puVar13 = puVar13 + 1;
      *puVar4 = *puVar3;
    }
    uVar10 = uVar9 + 0x800;
    uVar6 = uVar10;
  } while ((uVar10 < 0xc000) || (uVar6 = uVar9 + 0x2800, uVar10 < 0xe000));
  FUN_1000_1fdf(in_BX,iVar19,in_CX);
  bVar7 = *(byte *)0x3482;
  *(byte *)0x3482 = bVar7 ^ 1;
  out(0xa6,bVar7 ^ 1);
                    /* WARNING: Bad instruction - Truncating control flow here */
  halt_baddata();
}


